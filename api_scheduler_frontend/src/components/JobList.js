...
export default function JobList(input) {
  ...
  const Row = ({ index, style }) => (
    <ListItem style={style} key={index} component="div" disablePadding className={index % 2 ? "ListItemOdd" : "ListItemEven"}>
      <ListItemButton onClick={() =>{console.log(itemsArray[index].label);input.sendStoredDict(itemsArray[index].label)}}>
      
        <ListItemText primary={itemsArray[index].label} />
      </ListItemButton>
